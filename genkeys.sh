...
openssl req -new  -key server.key -subj "/C=CZ/ST=CZ/O=mytest/CN=server.local" -out server.csr
...
openssl req -new -key client.key -subj "/C=CZ/ST=CZ/O=mytest/CN=client.local" -out client.csr
